class e extends HTMLElement{constructor(){super();let e=document.getElementById("news-item").content;this.attachShadow({mode:"open"}).appendChild(e.cloneNode(!0))}static get observedAttributes(){return["title","date","source","desc","url","img"]}connectedCallback(){this.update()}attributeChangedCallback(){this.update()}update(){let e=this.getAttribute("title"),t=this.getAttribute("date"),i=this.getAttribute("source"),a=this.getAttribute("desc"),s=this.getAttribute("url"),o=this.getAttribute("img");if(null!=this.shadowRoot){let n=this.shadowRoot.getElementById("title"),r=this.shadowRoot.getElementById("date"),A=this.shadowRoot.getElementById("source"),l=this.shadowRoot.getElementById("desc"),c=this.shadowRoot.getElementById("url"),u=this.shadowRoot.getElementById("img");null!=n&&(n.textContent=e),null!=r&&(r.textContent=t),null!=A&&(A.textContent=i),null!=l&&(l.textContent=a),null!=c&&null!=s&&(c.href=s),null!=u&&null!=o&&(u.src=o)}}}customElements.define("news-item",e),async function(){let e=document.getElementById("news-list");JSON.parse(`
    {
        "status": "success",
        "totalResults": 169300,
        "results": [
            ...
            "title": "IBPS Officer Scale I, Assistant Officer posts provisional result out; here\u{2019}s direct link",
            ...
            "title": "Today\u{2019}s daily horoscope for October 30, 2024",
            ...
            "video_url": null,
            "description": "Aamir Khan was surprised and somewhat irked by Mira Nair\u{2019}s open declaration of her film Monsoon Wedding's superior chances at the Oscars due to her film\u{2019}s US exhibitors' lobbying powers.",
            "content": "ONLY AVAILABLE IN PAID PLANS",
    ...
    `).results.forEach(t=>{let i=document.createElement("news-item");i.setAttribute("title",t.title),i.setAttribute("date",t.pubDate),i.setAttribute("source",t.source_name),i.setAttribute("url",t.link),t.image_url&&i.setAttribute("desc",t.description),t.image_url&&i.setAttribute("img",t.image_url),null!=e&&e.appendChild(i)})}();
//# sourceMappingURL=news.949df1af.js.map
